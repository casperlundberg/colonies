...
// Type aliases so callers can use etcd.Node etc. while the underlying types
// are the canonical cluster package types.
type Node = cluster.Node
type Config = cluster.Config
type RelayMessage = cluster.RelayMessage
type MessageHandler = cluster.MessageHandler
