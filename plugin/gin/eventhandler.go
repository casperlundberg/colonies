...
func CreateEventHandler(relayServer cluster.Relay) backends.RealtimeEventHandler {
...
func CreateTestableEventHandler(relayServer cluster.Relay) backends.TestableRealtimeEventHandler {
	handler := &DefaultEventHandler{}
	handler.listeners = make(map[string]map[string]chan *core.Process)
	handler.processIDs = make(map[string]string)
	handler.nextExecutor = make(map[string]int)
